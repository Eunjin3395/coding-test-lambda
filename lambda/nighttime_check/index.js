...
const handler = async () => {
  ...
  const deadline1 = dayjs.tz(`${today} 22:01:00`, "Asia/Seoul");
  const deadline2 = dayjs.tz(`${today} 22:31:00`, "Asia/Seoul");
  const resultSummary = [];
  ...
  // ✅ MESSAGE_HISTORY에서 오전 메시지 정보 가져오기
  const getResult = await dynamo.get({ TableName: MESSAGE_HISTORY_TABLE, Key: { date: today } }).promise();
  const existingMessageId = getResult.Item?.messageId || [];
  const existingSentAt = getResult.Item?.sentAt || [];

  const updatedMessageId = [...existingMessageId, messageId];
  const updatedSentAt = [...existingSentAt, existingSentAt];
